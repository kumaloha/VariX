...
func TestSQLiteStore_RunNextMemoryOrganizationJobPrefersGraphFirstVerdictsWhenAvailable(t *testing.T) {
	...
	record := compile.Record{
		...
		Output: compile.Output{
			...
			Graph: compile.ReasoningGraph{
				Nodes: []compile.GraphNode{
					{ID: "n1", Kind: compile.NodeFact, Text: "美联储加息0.25%", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
					{ID: "n2", Kind: compile.NodePrediction, Text: "未来一周美股承压", PredictionStartAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC), PredictionDueAt: time.Date(2026, 4, 17, 0, 0, 0, 0, time.UTC)},
				},
				Edges: []compile.GraphEdge{{From: "n1", To: "n2", Kind: compile.EdgePositive}},
			},
			Details: compile.HiddenDetails{Caveats: []string{"detail"}},
	...
	if err := store.ApplyVerifyVerdictToContentSubgraph(context.Background(), "twitter", "Q-graph-first-organizer", graphmodel.VerifyVerdict{ObjectType: graphmodel.VerifyQueueObjectNode, ObjectID: "n2", Verdict: graphmodel.VerificationProved, Reason: "observed drawdown", AsOf: time.Date(2026, 4, 18, 0, 0, 0, 0, time.UTC).Format(time.RFC3339)}); err != nil {
	...
	if len(out.PredictionStatuses) != 1 || out.PredictionStatuses[0].Status != string(compile.PredictionStatusResolvedTrue) {
...
func TestSQLiteStore_RunNextMemoryOrganizationJobPrefersGraphFirstNodeTextByNodeID(t *testing.T) {
	...
	record := compile.Record{
		...
		Output: compile.Output{
			Summary: "summary",
			Graph: compile.ReasoningGraph{
				Nodes: []compile.GraphNode{{ID: "n1", Kind: compile.NodeFact, Text: "旧节点文本", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)}},
				Edges: []compile.GraphEdge{{From: "n1", To: "n1", Kind: compile.EdgeExplains}},
			},
			Details: compile.HiddenDetails{Caveats: []string{"detail"}},
		},
		CompiledAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC),
	...
	record.Output.Graph.Nodes = append(record.Output.Graph.Nodes, compile.GraphNode{ID: "n2", Kind: compile.NodeConclusion, Text: "结论", ValidFrom: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC), ValidTo: time.Date(2026, 4, 20, 0, 0, 0, 0, time.UTC)})
	record.Output.Graph.Edges = []compile.GraphEdge{{From: "n1", To: "n2", Kind: compile.EdgePositive}}
	if err := store.UpsertCompiledOutput(context.Background(), record); err != nil {
		t.Fatalf("UpsertCompiledOutput() error = %v", err)
	}
	...
	custom := graphmodel.ContentSubgraph{
		...
		Nodes: []graphmodel.GraphNode{
			{ID: "n1", SourceArticleID: "twitter:Q-graph-first-text", SourcePlatform: "twitter", SourceExternalID: "Q-graph-first-text", RawText: "更新后的节点文本", SubjectText: "更新后的节点文本", ChangeText: "更新后的节点文本", Kind: graphmodel.NodeKindObservation, IsPrimary: true, VerificationStatus: graphmodel.VerificationPending},
			{ID: "n2", SourceArticleID: "twitter:Q-graph-first-text", SourcePlatform: "twitter", SourceExternalID: "Q-graph-first-text", RawText: "结论", SubjectText: "结论", ChangeText: "结论", Kind: graphmodel.NodeKindObservation, IsPrimary: true, VerificationStatus: graphmodel.VerificationPending},
		},
...
func TestSQLiteStore_RunNextMemoryOrganizationJobPrefersGraphFirstValidityWindowByNodeID(t *testing.T) {
	...
	record := compile.Record{
		...
		Output: compile.Output{
			...
			Graph: compile.ReasoningGraph{
				Nodes: []compile.GraphNode{
					{ID: "n1", Kind: compile.NodePrediction, Text: "未来一周美股承压", PredictionStartAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC), PredictionDueAt: time.Date(2026, 4, 17, 0, 0, 0, 0, time.UTC)},
					{ID: "n2", Kind: compile.NodeConclusion, Text: "结论", ValidFrom: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC), ValidTo: time.Date(2026, 4, 20, 0, 0, 0, 0, time.UTC)},
				},
				Edges: []compile.GraphEdge{{From: "n1", To: "n2", Kind: compile.EdgePositive}},
			},
			Details: compile.HiddenDetails{Caveats: []string{"detail"}},
	...
	custom := graphmodel.ContentSubgraph{
		...
		UpdatedAt:        time.Date(2026, 4, 18, 0, 0, 0, 0, time.UTC).Format(time.RFC3339),
		Nodes: []graphmodel.GraphNode{
			{ID: "n1", SourceArticleID: "twitter:Q-graph-first-validity", SourcePlatform: "twitter", SourceExternalID: "Q-graph-first-validity", RawText: "未来一周美股承压", SubjectText: "美股", ChangeText: "承压", Kind: graphmodel.NodeKindPrediction, IsPrimary: true, VerificationStatus: graphmodel.VerificationPending, TimeStart: time.Date(2026, 4, 12, 0, 0, 0, 0, time.UTC).Format(time.RFC3339), TimeEnd: time.Date(2026, 4, 19, 0, 0, 0, 0, time.UTC).Format(time.RFC3339)},
			{ID: "n2", SourceArticleID: "twitter:Q-graph-first-validity", SourcePlatform: "twitter", SourceExternalID: "Q-graph-first-validity", RawText: "结论", SubjectText: "结论", ChangeText: "结论", Kind: graphmodel.NodeKindObservation, IsPrimary: true, VerificationStatus: graphmodel.VerificationPending},
		},
		Edges: []graphmodel.GraphEdge{{ID: "e1", From: "n1", To: "n2", Type: graphmodel.EdgeTypeDrives, IsPrimary: true, VerificationStatus: graphmodel.VerificationPending}},
	}
...
func TestSQLiteStore_RunNextMemoryOrganizationJobPrefersGraphFirstEdgesForHierarchy(t *testing.T) {
	...
	record := compile.Record{
		...
		Output: compile.Output{
			...
			Graph: compile.ReasoningGraph{
				Nodes: []compile.GraphNode{
					{ID: "n1", Kind: compile.NodeFact, Text: "事实A", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
					{ID: "n2", Kind: compile.NodeImplicitCondition, Text: "机制B", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
					{ID: "n3", Kind: compile.NodeConclusion, Text: "结论C", ValidFrom: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC), ValidTo: time.Date(2026, 4, 20, 0, 0, 0, 0, time.UTC)},
				},
				Edges: []compile.GraphEdge{{From: "n1", To: "n3", Kind: compile.EdgePositive}},
			},
			Details:      compile.HiddenDetails{Caveats: []string{"detail"}},
			Verification: compile.Verification{FactChecks: []compile.FactCheck{{NodeID: "n1", Status: compile.FactStatusClearlyTrue}}},
		},
		CompiledAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC),
	}
	if err := store.UpsertCompiledOutput(context.Background(), record); err != nil {
	...
	custom := graphmodel.ContentSubgraph{
		...
		UpdatedAt:        time.Date(2026, 4, 18, 0, 0, 0, 0, time.UTC).Format(time.RFC3339),
		Nodes: []graphmodel.GraphNode{
			{ID: "n1", SourceArticleID: "twitter:Q-graph-first-edges", SourcePlatform: "twitter", SourceExternalID: "Q-graph-first-edges", RawText: "事实A", SubjectText: "事实A", ChangeText: "事实A", Kind: graphmodel.NodeKindObservation, IsPrimary: true, VerificationStatus: graphmodel.VerificationPending},
			{ID: "n2", SourceArticleID: "twitter:Q-graph-first-edges", SourcePlatform: "twitter", SourceExternalID: "Q-graph-first-edges", RawText: "机制B", SubjectText: "机制B", ChangeText: "机制B", Kind: graphmodel.NodeKindObservation, IsPrimary: true, VerificationStatus: graphmodel.VerificationPending},
			{ID: "n3", SourceArticleID: "twitter:Q-graph-first-edges", SourcePlatform: "twitter", SourceExternalID: "Q-graph-first-edges", RawText: "结论C", SubjectText: "结论C", ChangeText: "结论C", Kind: graphmodel.NodeKindObservation, IsPrimary: true, VerificationStatus: graphmodel.VerificationPending},
		},
		Edges: []graphmodel.GraphEdge{{ID: "e1", From: "n1", To: "n2", Type: graphmodel.EdgeTypeDrives, IsPrimary: true, VerificationStatus: graphmodel.VerificationPending}, {ID: "e2", From: "n2", To: "n3", Type: graphmodel.EdgeTypeDrives, IsPrimary: true, VerificationStatus: graphmodel.VerificationPending}},
	}
