...
func (c *Client) Compile(ctx context.Context, bundle Bundle) (Record, error) {
	...
	stageStart = time.Now()
	graph = stageLedger(graph)
	recordStageMetric(stageMetrics, "ledger", time.Since(stageStart))
	debugStage(bundle, "ledger", fmt.Sprintf("done items=%d", len(graph.Ledger.Items)))
	c.writeDebugJSON(debugRunDir, "ledger.json", graph)
	stageStart = time.Now()
