...
func renderTransmissionPathDetails(paths []renderedPath, cn func(id, fallback string) string) []map[string]any {
	...
	for _, path := range paths {
		...
		entry := map[string]any{
			...
			"steps":         CloneStrings(transmission.Steps),
...
func confidenceFromState(drivers, targets []string, paths []TransmissionPath) string {
...
func splitParagraphs(text string) []string {
	...
	for _, item := range raw {
		...
		if isTextContextSectionMarker(item) {
			flush()
			continue
		}
...
func isTextContextSectionMarker(item string) bool {
	if len(item) < 3 || item[0] != '[' || item[len(item)-1] != ']' {
		return false
	}
	inner := strings.TrimSpace(item[1 : len(item)-1])
	if inner == "" {
		return false
	}
	parts := strings.Fields(strings.ToUpper(inner))
	if len(parts) == 0 {
		return false
	}
	switch parts[0] {
	...
	case "ROOT":
		return len(parts) == 2 && parts[1] == "CONTENT"
	default:
		return false
	}
}
