...
const authorValidationVersion = "author_claim_validation"
...
type authorVerificationPlan = AuthorVerificationPlan

type authorClaimVerificationPlan = AuthorClaimVerificationPlan

type authorInferenceVerificationPlan = AuthorInferenceVerificationPlan
...
func (c *Client) AuthorValidatePreviewResult(ctx context.Context, bundle Bundle, result FlowPreviewResult) (FlowPreviewResult, error) {
...
func runAuthorValidation(ctx context.Context, rt runtimeChat, model string, bundle Bundle, result FlowPreviewResult) (AuthorValidation, error) {
	...
	if err != nil {
		return AuthorValidation{}, fmt.Errorf("plan: %w", err)
	}
	...
	payload := map[string]any{
		...
		"status_contract": map[string]any{
			"claim_status": []string{
				string(AuthorClaimSupported),
				string(AuthorClaimContradicted),
				string(AuthorClaimUnverified),
				string(AuthorClaimInterpretive),
				string(AuthorClaimNotAuthorClaim),
			},
			"inference_status": []string{
				string(AuthorInferenceSound),
				string(AuthorInferenceWeak),
				string(AuthorInferenceUnsupportedJump),
				string(AuthorInferenceNotAuthorInference),
			},
		},
		...
		"url":                  bundle.URL,
		"current_date":         NowUTC().Format("2006-01-02"),
	...
	if err != nil {
		return AuthorValidation{}, err
	}
	req, err := varixllm.BuildProviderRequest(model, bundle, authorValidationSystemPrompt(), string(encoded), true)
	if err != nil {
		return AuthorValidation{}, err
	}
	var parsed AuthorValidation
	if err := callAuthorValidationJSON(ctx, rt, req, &parsed); err != nil {
		return AuthorValidation{}, fmt.Errorf("parse author validation output: %w", err)
	}
...
func runAuthorVerificationPlan(ctx context.Context, rt runtimeChat, model string, bundle Bundle, result FlowPreviewResult, claims []authorClaimCandidate, inferences []authorInferenceCandidate) (authorVerificationPlan, error) {
	payload := map[string]any{
	...
	req, err := varixllm.BuildProviderRequest(model, bundle, authorValidationPlanSystemPrompt(), string(encoded), false)
	if err != nil {
		return authorVerificationPlan{}, err
	}
