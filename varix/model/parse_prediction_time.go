...
func inferPredictionDueAtFromCalendarWindow(text string, start time.Time) (time.Time, bool) {
	...
	case containsBoundedPhrase(text, "明年") && !containsAnyPredictionPhrase(text, "明年后", "明年以后", "明年之后", "明年起", "明年开始"):
...
func containsAnyPredictionPhrase(text string, phrases ...string) bool {
	for _, phrase := range phrases {
		if strings.Contains(text, phrase) {
			return true
		}
