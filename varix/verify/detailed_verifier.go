...
func verifyNodeBatchDetailed(ctx context.Context, rt verifierCall, model string, prompts *promptRegistry, bundle Bundle, nodes []GraphNode) ([]NodeVerification, error) {
	...
	bullResp, _, err := callVerifierStage(ctx, rt, promptNodeVerifierBull, bullReq)
	if err != nil {
		return nil, err
	}
	...
	bearResp, _, err := callVerifierStage(ctx, rt, promptNodeVerifierBear, bearReq)
	if err != nil {
		return nil, err
	}
	...
	judgeResp, _, err := callVerifierStage(ctx, rt, promptNodeVerifierJudge, judgeReq)
	if err != nil {
		return nil, err
	}
...
func verifyPathBatchDetailed(ctx context.Context, rt verifierCall, model string, prompts *promptRegistry, bundle Bundle, output Output, nodeVerifications []NodeVerification, batch []TransmissionPath) ([]PathVerification, error) {
	...
	bullResp, _, err := callVerifierStage(ctx, rt, promptPathVerifierBull, bullReq)
	if err != nil {
		return nil, err
	}
	var bullPayload struct {
	...
	bearResp, _, err := callVerifierStage(ctx, rt, promptPathVerifierBear, bearReq)
	if err != nil {
		return nil, err
	}
	var bearPayload map[string]any
	...
	judgeResp, _, err := callVerifierStage(ctx, rt, promptPathVerifierJudge, judgeReq)
	if err != nil {
		return nil, err
	}
	var judgePayload struct {
