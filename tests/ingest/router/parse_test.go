...
func TestParse_TwitterPostCanonicalizesAuthorHandle(t *testing.T) {
	got, err := Parse("https://x.com/ElonMusk/status/1234567890")
	if err != nil {
		t.Fatalf("Parse() error = %v", err)
	}
	if got.AuthorID != "elonmusk" {
		t.Fatalf("AuthorID = %q, want lowercase canonical handle", got.AuthorID)
	}
}
