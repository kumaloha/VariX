...
func TestCollectorFetchRejectsEmptyStatusPayload(t *testing.T) {
	c := New(&fakeHTTPClient{
		status: weiboStatus{
			ID:     "125",
			Bid:    "Empty1",
	...
	_, err := c.Fetch(context.Background(), types.ParsedURL{
		...
		ContentType:  types.ContentTypePost,
		PlatformID:   "Empty1",
		CanonicalURL: "https://weibo.com/u1/Empty1",
	})
	if err == nil {
		t.Fatal("Fetch() error = nil, want empty status error")
	}
	if !strings.Contains(err.Error(), "empty weibo status") {
		t.Fatalf("Fetch() error = %v, want empty weibo status", err)
	}
}

func TestHTTPJSONClientFetchPostAcceptsStringPageInfoType(t *testing.T) {
		...
		Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			...
			case req.URL.Host == "weibo.com" && req.URL.Path == "/ajax/statuses/show":
				return textResponse(http.StatusOK, `{
					...
					"mblogid":"MooVNnFzc",
					"text_raw":"hello",
					"user":{"idstr":"6315229624","screen_name":"Alice"},
					"page_info":{"type":"video","object_type":"video","media_info":{}}
	...
	if got.TextRaw != "hello" {
		t.Fatalf("TextRaw = %q, want hello", got.TextRaw)
	}
}
