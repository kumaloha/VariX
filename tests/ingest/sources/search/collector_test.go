...
func TestCollector_DiscoverReportsBlockedOrUnparseableProviderResults(t *testing.T) {
	searcher := &fakeSearcher{
		body: `<html><body>If you're having trouble accessing Google Search, please <a href="/search?q=site:x.com/robin_j_brooks/status&emsg=SG_REL">click here</a></body></html>`,
	}
	c := New(types.PlatformTwitter, "x.com", searcher)
	...
	if err == nil {
		t.Fatal("Discover() error = nil, want diagnostic no-results error")
	}
	var noResults *NoUsableResultsError
	if !errors.As(err, &noResults) {
		t.Fatalf("Discover() error = %T, want *NoUsableResultsError", err)
	}
	if len(noResults.Attempts) == 0 {
		t.Fatalf("Attempts = %#v, want provider attempt diagnostics", noResults.Attempts)
	}
	if !noResults.Attempts[0].BlockedOrUnparseable {
		t.Fatalf("Attempts[0] = %#v, want blocked/unparseable classification", noResults.Attempts[0])
	}
	if !strings.Contains(err.Error(), "provider_blocked_or_unparseable") {
		t.Fatalf("error = %q, want provider_blocked_or_unparseable detail", err.Error())
	}
}

func TestCollector_DiscoverReportsFilteredProviderResults(t *testing.T) {
	searcher := &fakeSearcher{
		body: `<rss><channel>
<item><link>https://robinhood.com/login</link></item>
<item><link>https://x.com/search?q=robin_j_brooks</link></item>
</channel></rss>`,
	}
	c := New(types.PlatformTwitter, "x.com", searcher)
	...
	if len(noResults.Attempts) == 0 || noResults.Attempts[0].ExtractedCount == 0 || noResults.Attempts[0].RejectedByConstraintCount == 0 {
		t.Fatalf("Attempts = %#v, want extracted and constraint rejection diagnostics", noResults.Attempts)
	}
	if !strings.Contains(err.Error(), "provider_results_filtered") {
		t.Fatalf("error = %q, want provider_results_filtered detail", err.Error())
	}
}
...
<item><link>https://x.com/search?q=a</link></item>
<item><link>https://x.com/a/status/789</link></item>
<item><link>https://x.com/b/status/999</link></item>
</channel></rss>`,
	}
	c := New(types.PlatformTwitter, "x.com", searcher)
...
func TestCollector_DiscoverExtractsDuckDuckGoRedirectResults(t *testing.T) {
	searcher := &fakeSearcher{
		body: `<html><body>
<a class="result__a" href="//duckduckgo.com/l/?uddg=https%3A%2F%2Fx.com%2Frobin_j_brooks%2Fstatus%2F1750893106554159134&amp;rut=abc">result</a>
</body></html>`,
	}
	c := New(types.PlatformTwitter, "x.com", searcher)
	...
	if got[0].URL != "https://x.com/robin_j_brooks/status/1750893106554159134" {
		t.Fatalf("URL = %q, want DuckDuckGo redirect target", got[0].URL)
	}
}

func TestCollector_DiscoverUsesTwitterAuthorRSSFallback(t *testing.T) {
	fallback := NewNitterAuthorRSSSearcher(&http.Client{
		Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			if req.URL.String() != "https://nitter.net/robin_j_brooks/rss" {
				t.Fatalf("nitter request URL = %q, want author RSS", req.URL.String())
			}
			return &http.Response{
				StatusCode: http.StatusOK,
				Header:     make(http.Header),
				Body: io.NopCloser(strings.NewReader(`<rss><channel>
<item><link>https://nitter.net/other_author/status/111#m</link></item>
<item><link>https://nitter.net/robin_j_brooks/status/2050327284154786020#m</link></item>
</channel></rss>`)),
			}, nil
		}),
	})
	c := New(types.PlatformTwitter, "x.com", &fakeSearcher{body: `<html></html>`}, fallback)
	...
	if got[0].URL != "https://x.com/robin_j_brooks/status/2050327284154786020" {
		t.Fatalf("URL = %q, want rewritten author status URL", got[0].URL)
	}
}

func TestCollector_DiscoverTreatsXAndTwitterSiteConstraintsAsEquivalentForTwitter(t *testing.T) {
	searcher := &fakeSearcher{
		body: `<rss><channel>
<item><link>https://x.com/robin_j_brooks/status/2050327284154786020</link></item>
</channel></rss>`,
	}
	c := New(types.PlatformTwitter, "twitter.com", searcher)

	got, err := c.Discover(context.Background(), types.FollowTarget{
		Kind:     types.KindSearch,
		Platform: "twitter",
		Query:    "site:twitter.com/robin_j_brooks/status",
	})
	if err != nil {
		t.Fatalf("Discover() error = %v", err)
	}
	...
	if got[0].URL != "https://x.com/robin_j_brooks/status/2050327284154786020" {
		t.Fatalf("URL = %q, want x.com result accepted for twitter.com constraint", got[0].URL)
	}
}
