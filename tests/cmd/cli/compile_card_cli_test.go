...
func TestRunCompileCardPrintsHumanReadableCard(t *testing.T) {
	...
	out := stdout.String()
	for _, want := range []string{"Summary", "一句话总结", "Mainline", "Topics", "topic-a", "Management declarations", "Greg Abel", "capital_allocation", "伯克希尔会等待市场错配", "Read: 这是伯克希尔的资本配置纪律", "平时以不会仅因现金规模大而被迫投资为边界", "触发条件是市场出现错配", "条件满足后快速且果断行动，规模是投入大量资本", "Condition: 市场出现错配", "Action: 快速且果断行动", "Scale: 投入大量资本", "Boundary: 不会仅因现金规模大而被迫投资", "Evidence: 现金和短债约3800亿美元", "Speaker claims", "existing portfolio / circle of competence", "Question: 股东询问 Greg Abel 如何管理 Warren Buffett 建立的组合。", "Answer: 现有组合由 Warren Buffett 建立", "Apple 说明能力圈不是行业标签", "Branches", "Anchor: 总前提", "Branch driver: 分支机制", "Declaration: 伯克希尔会等待市场错配", "驱动A -> 中间步骤 -> 目标B", "Side logic", "Author validation", "Verdict: mixed", "Claims: supported 1, contradicted 0, unverified 1, interpretive 0", "Claim 目标B: unverified — 说明: 缺少外部证据", "Path 驱动A -> 目标B: unsupported_jump — 说明: 中间条件不成立", "Confidence", "high"} {
