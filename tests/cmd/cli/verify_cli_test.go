...
func TestRunVerifyRunAndShowUseSeparateVerificationStore(t *testing.T) {
	...
	prevBuildVerifyClient := buildVerifyClient
	prevOpenSQLiteStore := openSQLiteStore
	t.Cleanup(func() {
		newIngestRuntime = prevNewIngestRuntime
		buildVerifyClient = prevBuildVerifyClient
	...
	buildVerifyClient = func(projectRoot string) verifyClient {
...
func TestRunVerifyQueueListPrintsQueuedItems(t *testing.T) {
	...
	openSQLiteStore = func(path string) (*contentstore.SQLiteStore, error) {
		...
		now := time.Now().UTC()
		if err := store.EnqueueVerifyQueueItem(context.Background(), model.VerifyQueueItem{ID: "q-cli-1", ObjectType: model.VerifyQueueObjectNode, ObjectID: "n1", SourceArticleID: "unit-cli", Priority: 10, ScheduledAt: now.Format(time.RFC3339), Status: model.VerifyQueueStatusQueued}); err != nil {
			return nil, err
		}
		if err := store.EnqueueVerifyQueueItem(context.Background(), model.VerifyQueueItem{ID: "q-cli-2", ObjectType: model.VerifyQueueObjectEdge, ObjectID: "e1", SourceArticleID: "unit-cli", Priority: 5, ScheduledAt: now.Format(time.RFC3339), Status: model.VerifyQueueStatusQueued}); err != nil {
			return nil, err
		}
...
func TestRunVerifySweepProcessesQueueFromCurrentContentGraphState(t *testing.T) {
	...
	openSQLiteStore = func(path string) (*contentstore.SQLiteStore, error) {
		...
		now := time.Now().UTC()
		subgraph := model.ContentSubgraph{ID: "sweep-cli", ArticleID: "sweep-cli", SourcePlatform: "twitter", SourceExternalID: "sweep-cli", CompileVersion: model.CompileBridgeVersion, CompiledAt: now.Format(time.RFC3339), UpdatedAt: now.Format(time.RFC3339), Nodes: []model.ContentNode{{ID: "n1", SourceArticleID: "sweep-cli", SourcePlatform: "twitter", SourceExternalID: "sweep-cli", RawText: "未来一周美股承压", SubjectText: "美股", ChangeText: "承压", Kind: model.NodeKindPrediction, GraphRole: model.GraphRoleTarget, IsPrimary: true, VerificationStatus: model.VerificationProved, VerificationReason: "resolved", VerificationAsOf: now.Format(time.RFC3339), TimeBucket: "1w"}}}
		if err := store.UpsertContentSubgraph(context.Background(), subgraph); err != nil {
			return nil, err
		}
		if err := store.EnqueueVerifyQueueItem(context.Background(), model.VerifyQueueItem{ID: "q-sweep-cli", ObjectType: model.VerifyQueueObjectNode, ObjectID: "n1", SourceArticleID: "sweep-cli", Priority: 10, ScheduledAt: now.Add(-time.Hour).Format(time.RFC3339), Status: model.VerifyQueueStatusQueued}); err != nil {
			return nil, err
		}
		return store, nil
	}
...
func TestRunVerifyQueueSupportsStatusFilter(t *testing.T) {
	...
	openSQLiteStore = func(path string) (*contentstore.SQLiteStore, error) {
		...
		now := time.Now().UTC()
		if err := store.EnqueueVerifyQueueItem(context.Background(), model.VerifyQueueItem{ID: "q-filter-1", ObjectType: model.VerifyQueueObjectNode, ObjectID: "n1", SourceArticleID: "unit-cli", Priority: 10, ScheduledAt: now.Format(time.RFC3339), Status: model.VerifyQueueStatusQueued}); err != nil {
			return nil, err
		}
		if err := store.EnqueueVerifyQueueItem(context.Background(), model.VerifyQueueItem{ID: "q-filter-2", ObjectType: model.VerifyQueueObjectEdge, ObjectID: "e1", SourceArticleID: "unit-cli", Priority: 5, ScheduledAt: now.Format(time.RFC3339), Status: model.VerifyQueueStatusQueued}); err != nil {
			return nil, err
		}
...
func TestRunVerifyQueueSummaryPrintsCounts(t *testing.T) {
	...
	openSQLiteStore = func(path string) (*contentstore.SQLiteStore, error) {
		...
		now := time.Now().UTC()
		if err := store.EnqueueVerifyQueueItem(context.Background(), model.VerifyQueueItem{ID: "q-summary-1", ObjectType: model.VerifyQueueObjectNode, ObjectID: "n1", SourceArticleID: "unit-cli", Priority: 10, ScheduledAt: now.Format(time.RFC3339), Status: model.VerifyQueueStatusQueued}); err != nil {
			return nil, err
		}
		if err := store.EnqueueVerifyQueueItem(context.Background(), model.VerifyQueueItem{ID: "q-summary-2", ObjectType: model.VerifyQueueObjectEdge, ObjectID: "e1", SourceArticleID: "unit-cli", Priority: 5, ScheduledAt: now.Format(time.RFC3339), Status: model.VerifyQueueStatusQueued}); err != nil {
			return nil, err
		}
...
func TestRunVerifyShowFallsBackToGraphFirstVerificationState(t *testing.T) {
	...
	openSQLiteStore = func(path string) (*contentstore.SQLiteStore, error) {
		...
		subgraph := model.ContentSubgraph{ID: "verify-show-fallback", ArticleID: "verify-show-fallback", SourcePlatform: "twitter", SourceExternalID: "verify-show-fallback", CompileVersion: model.CompileBridgeVersion, CompiledAt: time.Now().UTC().Format(time.RFC3339), UpdatedAt: time.Now().UTC().Format(time.RFC3339), Nodes: []model.ContentNode{{ID: "n1", SourceArticleID: "verify-show-fallback", SourcePlatform: "twitter", SourceExternalID: "verify-show-fallback", RawText: "事实A", SubjectText: "事实A", ChangeText: "事实A", Kind: model.NodeKindObservation, IsPrimary: true, VerificationStatus: model.VerificationProved, VerificationReason: "supported", VerificationAsOf: time.Now().UTC().Format(time.RFC3339)}, {ID: "n2", SourceArticleID: "verify-show-fallback", SourcePlatform: "twitter", SourceExternalID: "verify-show-fallback", RawText: "未来一周结论B", SubjectText: "结论B", ChangeText: "未来一周结论B", Kind: model.NodeKindPrediction, IsPrimary: true, VerificationStatus: model.VerificationPending, VerificationReason: "waiting", VerificationAsOf: time.Now().UTC().Format(time.RFC3339), NextVerifyAt: time.Now().UTC().Add(time.Hour).Format(time.RFC3339)}}}
		if err := store.UpsertContentSubgraph(context.Background(), subgraph); err != nil {
			return nil, err
		}
		return store, nil
