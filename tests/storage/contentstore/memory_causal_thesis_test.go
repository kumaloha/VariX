...
func TestBuildCausalThesis_AssignsRoles(t *testing.T) {
	...
	nodesByID := map[string]memory.AcceptedNode{
		"weibo:S1:n1": {NodeID: "weibo:S1:n1", NodeKind: string(model.NodeFact), NodeText: "流动性收紧"},
		"weibo:S1:n2": {NodeID: "weibo:S1:n2", NodeKind: string(model.NodeConclusion), NodeText: "风险资产承压"},
		"weibo:S1:n3": {NodeID: "weibo:S1:n3", NodeKind: string(model.NodePrediction), NodeText: "未来数月波动加大"},
	}
...
func TestBuildCausalThesis_PreservesConditionAndMechanismInCorePath(t *testing.T) {
	...
	nodesByID := map[string]memory.AcceptedNode{
		"weibo:S1:n1": {NodeID: "weibo:S1:n1", NodeKind: string(model.NodeFact), NodeText: "流动性收紧"},
		"weibo:S1:n2": {NodeID: "weibo:S1:n2", NodeKind: string(model.NodeExplicitCondition), NodeText: "若融资环境继续恶化"},
		"weibo:S1:n3": {NodeID: "weibo:S1:n3", NodeKind: string(model.NodeImplicitCondition), NodeText: "高杠杆会放大资产价格脆弱性"},
		"weibo:S1:n4": {NodeID: "weibo:S1:n4", NodeKind: string(model.NodeConclusion), NodeText: "风险资产承压"},
		"weibo:S1:n5": {NodeID: "weibo:S1:n5", NodeKind: string(model.NodePrediction), NodeText: "未来数月波动加大"},
	}
...
func TestBuildCausalThesis_PreservesTransmissionBridgeNodeInCorePath(t *testing.T) {
	...
	nodesByID := map[string]memory.AcceptedNode{
		"weibo:G04:n1": {NodeID: "weibo:G04:n1", NodeKind: string(model.NodeFact), NodeText: "海外资金继续流入美国资产"},
		"weibo:G04:n2": {NodeID: "weibo:G04:n2", NodeKind: string(model.NodeMechanism), NodeText: "增长与回报预期仍压过政治风险并维持美国资产配置偏好"},
		"weibo:G04:n3": {NodeID: "weibo:G04:n3", NodeKind: string(model.NodeConclusion), NodeText: "当前并不存在 sell America trade"},
	}
