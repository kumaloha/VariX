...
func TestSQLiteStore_PersistMemoryContentGraphDeferredMarksDirtyWithoutProjectionRefresh(t *testing.T) {
	...
	graph := subjectTimelineSubgraph("deferred", now, []model.ContentNode{
		subjectHorizonNode("driver", "deferred", "油价", "继续上涨", now, model.GraphRoleDriver),
		subjectHorizonNode("target", "deferred", "美股", "从纪录高位回落", now, model.GraphRoleTarget),
	})
...
func TestSQLiteStore_RunProjectionDirtySweepRefreshesDeferredMarks(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Date(2026, 4, 30, 12, 0, 0, 0, time.UTC)
	graph := subjectTimelineSubgraph("sweep", now, []model.ContentNode{
		subjectHorizonNode("driver", "sweep", "油价", "继续上涨", now, model.GraphRoleDriver),
		subjectHorizonNode("target", "sweep", "美股", "从纪录高位回落", now, model.GraphRoleTarget),
	})
...
func TestSQLiteStore_RunProjectionDirtySweepAvoidsDuplicateBaseProjectionRefresh(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Date(2026, 4, 30, 12, 0, 0, 0, time.UTC)
	graph := subjectTimelineSubgraph("dedupe", now, []model.ContentNode{
		subjectHorizonNode("driver", "dedupe", "油价", "继续上涨", now, model.GraphRoleDriver),
		subjectHorizonNode("target", "dedupe", "美股", "从纪录高位回落", now, model.GraphRoleTarget),
	})
	...
	for _, mark := range marks {
		switch mark.Layer {
		case "event", "paradigm", "global-synthesis":
		default:
...
func TestSQLiteStore_RunProjectionDirtyMarkStoresRefreshedSubjectHorizonForExperience(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Date(2026, 4, 30, 12, 0, 0, 0, time.UTC)
	graph := subjectTimelineSubgraph("state-horizon", now, []model.ContentNode{
		subjectHorizonNode("target", "state-horizon", "美股", "继续走强", now, model.GraphRoleTarget),
	})
...
func TestProjectionDirtyUserStateSubjectGraphsCoalescesConcurrentLoads(t *testing.T) {
	...
	var loadCalls int32
	load := func(ctx context.Context, userID, subject string) ([]model.ContentSubgraph, error) {
		...
		return []model.ContentSubgraph{{SourceExternalID: "source-1"}}, nil
	}
	done := make(chan []model.ContentSubgraph, 2)
...
func TestProjectionDirtyUserStateCanonicalSubjectsReuseLookup(t *testing.T) {
	...
	state := &projectionDirtyUserState{}
	var calls int32
	resolve := func(context.Context, model.ContentNode, map[string]string) (string, error) {
		atomic.AddInt32(&calls, 1)
		return "美联储", nil
	}
	node := model.ContentNode{SubjectText: "联储"}
