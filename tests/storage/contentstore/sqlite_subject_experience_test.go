...
func TestSQLiteStore_GetSubjectExperienceMemoryDerivesReusableDriverLessonsAcrossHorizons(t *testing.T) {
	...
	for _, graph := range []model.ContentSubgraph{
		subjectTimelineSubgraph("experience-recent", recent, []model.ContentNode{
			subjectHorizonNode("recent-driver", "experience-recent", "油价", "油价上行压制风险偏好", recent, model.GraphRoleDriver),
			subjectHorizonNode("recent-target", "experience-recent", "美股", "从纪录高位回落", recent, model.GraphRoleTarget),
		}),
		subjectTimelineSubgraph("experience-older", older, []model.ContentNode{
			subjectHorizonNode("older-driver", "experience-older", "油价", "能源价格冲击估值", older, model.GraphRoleDriver),
			subjectHorizonNode("older-target", "experience-older", "美股", "估值承压", older, model.GraphRoleTarget),
		}),
	} {
...
func TestSQLiteStore_GetSubjectExperienceMemoryUsesGraphPathWhenMechanismEvidenceExists(t *testing.T) {
	store := newSubjectTimelineTestStore(t)
	now := time.Date(2026, 4, 30, 12, 0, 0, 0, time.UTC)
	graph := subjectTimelineSubgraph("experience-path", now, []model.ContentNode{
		subjectHorizonNode("oil", "experience-path", "油价", "上涨", now, model.GraphRoleDriver),
		subjectHorizonNode("cpi", "experience-path", "CPI", "通胀预期升温", now, model.GraphRoleIntermediate),
		subjectHorizonNode("rate", "experience-path", "利率", "降息预期下降", now, model.GraphRoleIntermediate),
		subjectHorizonNode("target", "experience-path", "美股", "估值承压", now, model.GraphRoleTarget),
	})
	graph.Edges = []model.ContentEdge{
		{ID: "e1", From: "oil", To: "cpi", Type: model.EdgeTypeDrives, IsPrimary: true},
		{ID: "e2", From: "cpi", To: "rate", Type: model.EdgeTypeDrives, IsPrimary: true},
		{ID: "e3", From: "rate", To: "target", Type: model.EdgeTypeDrives, IsPrimary: true},
	}
...
func TestSQLiteStore_GetSubjectExperienceMemoryReusesFreshCacheWhenHorizonInputsUnchanged(t *testing.T) {
	store := newSubjectTimelineTestStore(t)
	now := time.Date(2026, 4, 30, 12, 0, 0, 0, time.UTC)
	graph := subjectTimelineSubgraph("experience-cache", now, []model.ContentNode{
		subjectHorizonNode("driver", "experience-cache", "科技股", "科技股走强", now, model.GraphRoleDriver),
		subjectHorizonNode("target", "experience-cache", "美股", "继续创新高", now, model.GraphRoleTarget),
	})
