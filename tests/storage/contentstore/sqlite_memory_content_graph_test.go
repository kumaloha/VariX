...
func TestSQLiteStore_AcceptMemoryNodesDefersProjectionRefreshAndMarksDirty(t *testing.T) {
	...
	record := model.Record{
		...
		Output: model.Output{
			...
			Graph: model.ReasoningGraph{
				Nodes: []model.GraphNode{
					{ID: "n1", Kind: model.NodeFact, Text: "油价上涨", OccurredAt: time.Date(2026, 4, 21, 0, 0, 0, 0, time.UTC)},
					{ID: "n2", Kind: model.NodePrediction, Text: "美股承压", PredictionStartAt: time.Date(2026, 4, 21, 0, 0, 0, 0, time.UTC), PredictionDueAt: time.Date(2026, 4, 28, 0, 0, 0, 0, time.UTC)},
				},
				Edges: []model.GraphEdge{{From: "n1", To: "n2", Kind: model.EdgePositive}},
			},
			Details: model.HiddenDetails{Caveats: []string{"detail"}},
...
func TestSQLiteStore_AcceptMemoryNodesUpdatesExistingGraphFirstContentMemorySnapshot(t *testing.T) {
	...
	if err := store.ApplyVerifyVerdictToContentSubgraph(context.Background(), "twitter", "mg2", model.VerifyVerdict{ObjectType: model.VerifyQueueObjectNode, ObjectID: "n2", Verdict: model.VerificationProved, Reason: "observed drawdown", AsOf: time.Date(2026, 4, 29, 0, 0, 0, 0, time.UTC).Format(time.RFC3339)}); err != nil {
	...
	var verdict model.VerificationStatus
	for _, node := range got.Nodes {
		if node.ID == "n2" {
			verdict = node.VerificationStatus
		}
	}
	if verdict != model.VerificationProved {
...
func TestSQLiteStore_ApplyVerifyVerdictAlsoRefreshesMemoryContentGraphSnapshot(t *testing.T) {
	...
	if err := store.ApplyVerifyVerdictToContentSubgraph(context.Background(), "twitter", "mg3", model.VerifyVerdict{ObjectType: model.VerifyQueueObjectNode, ObjectID: "n2", Verdict: model.VerificationProved, Reason: "observed drawdown", AsOf: time.Date(2026, 4, 29, 0, 0, 0, 0, time.UTC).Format(time.RFC3339)}); err != nil {
		t.Fatalf("ApplyVerifyVerdictToContentSubgraph() error = %v", err)
	}
...
func TestSQLiteStore_PersistMemoryContentGraphAlsoProjectsEventAndParadigmLayers(t *testing.T) {
	...
	subgraph := model.ContentSubgraph{
		...
		Nodes: []model.ContentNode{
			{ID: "n1", SourceArticleID: "mg-auto-1", SourcePlatform: "twitter", SourceExternalID: "mg-auto-1", RawText: "美联储加息0.25%", SubjectText: "美联储", ChangeText: "加息0.25%", Kind: model.NodeKindObservation, GraphRole: model.GraphRoleDriver, IsPrimary: true, VerificationStatus: model.VerificationPending, TimeBucket: "1w"},
			{ID: "n2", SourceArticleID: "mg-auto-1", SourcePlatform: "twitter", SourceExternalID: "mg-auto-1", RawText: "未来一周美股承压", SubjectText: "美股", ChangeText: "承压", Kind: model.NodeKindPrediction, GraphRole: model.GraphRoleTarget, IsPrimary: true, VerificationStatus: model.VerificationProved, TimeBucket: "1w"},
		},
	}
...
func TestSQLiteStore_PersistMemoryContentGraphFromCompiledOutputPersistsBySource(t *testing.T) {
	...
	record := model.Record{
		...
		Output: model.Output{
			Summary: "summary text",
			Graph: model.ReasoningGraph{
				Nodes: []model.GraphNode{{ID: "n1", Kind: model.NodeFact, Text: "美联储加息0.25%", OccurredAt: time.Date(2026, 4, 21, 0, 0, 0, 0, time.UTC)}, {ID: "n2", Kind: model.NodePrediction, Text: "未来一周美股承压", PredictionStartAt: time.Date(2026, 4, 21, 0, 0, 0, 0, time.UTC), PredictionDueAt: time.Date(2026, 4, 28, 0, 0, 0, 0, time.UTC)}},
				Edges: []model.GraphEdge{{From: "n1", To: "n2", Kind: model.EdgePositive}},
			},
			Details: model.HiddenDetails{Caveats: []string{"detail"}},
		},
...
func TestSQLiteStore_ListMemoryContentGraphsBySubjectFiltersSnapshots(t *testing.T) {
	...
	for _, sg := range []model.ContentSubgraph{
		{ID: "subject-cg-1", ArticleID: "subject-cg-1", SourcePlatform: "twitter", SourceExternalID: "subject-cg-1", CompileVersion: model.CompileBridgeVersion, CompiledAt: now.Format(time.RFC3339), UpdatedAt: now.Format(time.RFC3339), Nodes: []model.ContentNode{{ID: "n1", SourceArticleID: "subject-cg-1", SourcePlatform: "twitter", SourceExternalID: "subject-cg-1", RawText: "美联储加息0.25%", SubjectText: "美联储", ChangeText: "加息0.25%", Kind: model.NodeKindObservation, IsPrimary: true, VerificationStatus: model.VerificationPending}}},
		{ID: "subject-cg-2", ArticleID: "subject-cg-2", SourcePlatform: "twitter", SourceExternalID: "subject-cg-2", CompileVersion: model.CompileBridgeVersion, CompiledAt: now.Format(time.RFC3339), UpdatedAt: now.Format(time.RFC3339), Nodes: []model.ContentNode{{ID: "n1", SourceArticleID: "subject-cg-2", SourcePlatform: "twitter", SourceExternalID: "subject-cg-2", RawText: "欧洲央行放缓缩表", SubjectText: "欧洲央行", ChangeText: "放缓缩表", Kind: model.NodeKindObservation, IsPrimary: true, VerificationStatus: model.VerificationPending}}},
	} {
...
func TestSQLiteStore_MemoryContentGraphSubjectsIndexTracksPersistAndUpdate(t *testing.T) {
	...
	second := first
	second.Nodes = []model.ContentNode{{
		ID:                 "n2",
		SourceArticleID:    "indexed-cg",
		SourcePlatform:     "twitter",
		SourceExternalID:   "indexed-cg",
...
func TestSQLiteStore_ListMemoryContentGraphsBySourceAndSubjectUsesIntersection(t *testing.T) {
	...
	now := time.Date(2026, 4, 21, 0, 0, 0, 0, time.UTC)
	for _, sg := range []model.ContentSubgraph{
		{ID: "cg-ss-1", ArticleID: "cg-ss-1", SourcePlatform: "twitter", SourceExternalID: "cg-ss-1", CompileVersion: model.CompileBridgeVersion, CompiledAt: now.Format(time.RFC3339), UpdatedAt: now.Format(time.RFC3339), Nodes: []model.ContentNode{{ID: "n1", SourceArticleID: "cg-ss-1", SourcePlatform: "twitter", SourceExternalID: "cg-ss-1", RawText: "美联储加息0.25%", SubjectText: "美联储", ChangeText: "加息0.25%", Kind: model.NodeKindObservation, IsPrimary: true, VerificationStatus: model.VerificationPending}}},
		{ID: "cg-ss-2", ArticleID: "cg-ss-2", SourcePlatform: "twitter", SourceExternalID: "cg-ss-2", CompileVersion: model.CompileBridgeVersion, CompiledAt: now.Format(time.RFC3339), UpdatedAt: now.Format(time.RFC3339), Nodes: []model.ContentNode{{ID: "n1", SourceArticleID: "cg-ss-2", SourcePlatform: "twitter", SourceExternalID: "cg-ss-2", RawText: "欧洲央行放缓缩表", SubjectText: "欧洲央行", ChangeText: "放缓缩表", Kind: model.NodeKindObservation, IsPrimary: true, VerificationStatus: model.VerificationPending}}},
	} {
...
func TestSQLiteStore_ListMemoryContentGraphsBySourceAndSubjectSupportsAliasLookup(t *testing.T) {
	...
	for _, sg := range []model.ContentSubgraph{
		{ID: "cg-alias-ss-1", ArticleID: "cg-alias-ss-1", SourcePlatform: "twitter", SourceExternalID: "cg-alias-ss-1", CompileVersion: model.CompileBridgeVersion, CompiledAt: now.Format(time.RFC3339), UpdatedAt: now.Format(time.RFC3339), Nodes: []model.ContentNode{{ID: "n1", SourceArticleID: "cg-alias-ss-1", SourcePlatform: "twitter", SourceExternalID: "cg-alias-ss-1", RawText: "美联储加息0.25%", SubjectText: "美联储", ChangeText: "加息0.25%", Kind: model.NodeKindObservation, IsPrimary: true, VerificationStatus: model.VerificationPending}}},
		{ID: "cg-alias-ss-2", ArticleID: "cg-alias-ss-2", SourcePlatform: "twitter", SourceExternalID: "cg-alias-ss-2", CompileVersion: model.CompileBridgeVersion, CompiledAt: now.Format(time.RFC3339), UpdatedAt: now.Format(time.RFC3339), Nodes: []model.ContentNode{{ID: "n1", SourceArticleID: "cg-alias-ss-2", SourcePlatform: "twitter", SourceExternalID: "cg-alias-ss-2", RawText: "欧洲央行放缓缩表", SubjectText: "欧洲央行", ChangeText: "放缓缩表", Kind: model.NodeKindObservation, IsPrimary: true, VerificationStatus: model.VerificationPending}}},
	} {
