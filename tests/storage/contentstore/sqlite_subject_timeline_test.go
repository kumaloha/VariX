...
func TestSQLiteStore_BuildSubjectTimelineGroupsChangesBySubject(t *testing.T) {
	...
	graphs := []model.ContentSubgraph{
		subjectTimelineSubgraph("sg-1", now, []model.ContentNode{{
			ID:                 "n1",
		...
		subjectTimelineSubgraph("sg-2", now, []model.ContentNode{{
			ID:                 "n2",
		...
		subjectTimelineSubgraph("sg-3", now, []model.ContentNode{{
			ID:                 "n3",
			...
			TimeStart:          now.Format(time.RFC3339),
			Kind:               model.NodeKindObservation,
			GraphRole:          model.GraphRoleDriver,
			IsPrimary:          true,
			VerificationStatus: model.VerificationPending,
		}}),
	}
	...
	if timeline.Entries[1].VerificationStatus != string(model.VerificationProved) || timeline.Entries[1].VerificationReason != "observed drawdown" {
...
func TestSQLiteStore_BuildSubjectTimelineMarksLegacyMirroredNodeLowStructure(t *testing.T) {
	store := newSubjectTimelineTestStore(t)
	now := time.Date(2026, 4, 30, 0, 0, 0, 0, time.UTC)
	graph := subjectTimelineSubgraph("legacy", now, []model.ContentNode{{
		ID:                 "n1",
...
func TestSQLiteStore_BuildSubjectTimelineClassifiesContradictoryChange(t *testing.T) {
	store := newSubjectTimelineTestStore(t)
	now := time.Date(2026, 4, 30, 0, 0, 0, 0, time.UTC)
	graph := subjectTimelineSubgraph("contradict", now, []model.ContentNode{
		{
			ID:                 "n1",
...
func TestSQLiteStore_BuildSubjectTimelineExcludesNonPrimaryAndContextNodes(t *testing.T) {
	store := newSubjectTimelineTestStore(t)
	now := time.Date(2026, 4, 30, 0, 0, 0, 0, time.UTC)
	graph := subjectTimelineSubgraph("roles", now, []model.ContentNode{
		{
			ID:                 "primary",
		...
		},
		{
			...
			ChangeText:         "历史估值偏高",
			Kind:               model.NodeKindObservation,
			GraphRole:          model.GraphRoleContext,
			IsPrimary:          true,
			VerificationStatus: model.VerificationPending,
		},
		{
...
func TestSQLiteStore_BuildSubjectTimelineUsesSubjectIndex(t *testing.T) {
	...
	graph := subjectTimelineSubgraph("timeline-no-index", now, []model.ContentNode{
		subjectHorizonNode("target", "timeline-no-index", "美股", "继续上涨", now, model.GraphRoleTarget),
	})
...
func subjectTimelineSubgraph(id string, now time.Time, nodes []model.ContentNode) model.ContentSubgraph {
