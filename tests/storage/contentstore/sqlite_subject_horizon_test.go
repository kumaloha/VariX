...
func TestSQLiteStore_GetSubjectHorizonMemoryBuildsRollingWindowAbstraction(t *testing.T) {
	...
	graphs := []model.ContentSubgraph{
		subjectTimelineSubgraph("old", old, []model.ContentNode{
			subjectHorizonNode("old-driver", "old", "科技股", "财报强劲", old, model.GraphRoleDriver),
			subjectHorizonNode("old-target", "old", "美股", "刷新历史高位", old, model.GraphRoleTarget),
		}),
		subjectTimelineSubgraph("recent", recent, []model.ContentNode{
			subjectHorizonNode("recent-driver", "recent", "油价", "油价继续上涨", recent, model.GraphRoleDriver),
			subjectHorizonNode("recent-target", "recent", "美股", "从纪录高位回落", recent, model.GraphRoleTarget),
		}),
	}
...
func TestSQLiteStore_GetSubjectHorizonMemoryReusesFreshCache(t *testing.T) {
	store := newSubjectTimelineTestStore(t)
	now := time.Date(2026, 4, 30, 12, 0, 0, 0, time.UTC)
	graph := subjectTimelineSubgraph("cached", now, []model.ContentNode{
		subjectHorizonNode("driver", "cached", "科技股", "科技股走强", now, model.GraphRoleDriver),
		subjectHorizonNode("target", "cached", "美股", "继续创新高", now, model.GraphRoleTarget),
	})
...
func TestSQLiteStore_GetSubjectHorizonMemoryClassifiesRelationsAfterSortingByTime(t *testing.T) {
	...
	for _, graph := range []model.ContentSubgraph{
		subjectTimelineSubgraph("z-early", early, []model.ContentNode{
			subjectHorizonNode("target", "z-early", "美股", "上涨并刷新高位", early, model.GraphRoleTarget),
		}),
		subjectTimelineSubgraph("a-late", late, []model.ContentNode{
			subjectHorizonNode("target", "a-late", "美股", "下跌并从高位回落", late, model.GraphRoleTarget),
		}),
	} {
...
func TestSQLiteStore_GetSubjectHorizonMemoryRefreshUsesSubjectIndex(t *testing.T) {
	...
	now := time.Date(2026, 4, 30, 12, 0, 0, 0, time.UTC)
	graph := subjectTimelineSubgraph("horizon-no-index", now, []model.ContentNode{
		subjectHorizonNode("target", "horizon-no-index", "美股", "继续上涨", now, model.GraphRoleTarget),
	})
...
func subjectHorizonNode(id, sourceID, subject, change string, at time.Time, role model.GraphRole) model.ContentNode {
