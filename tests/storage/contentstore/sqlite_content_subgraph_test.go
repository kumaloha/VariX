...
func testContentSubgraph() model.ContentSubgraph {
...
func TestSQLiteStore_UpsertCompiledOutputBridgesToContentSubgraphTable(t *testing.T) {
		...
		Output: model.Output{
			...
			Graph: model.ReasoningGraph{
				Nodes: []model.GraphNode{
					testCompileGraphNode("n1", model.NodeFact, "美联储加息0.25%"),
					testCompileGraphNode("n2", model.NodePrediction, "未来一周美股承压"),
				},
	...
	if len(got.Edges) != 1 || got.Edges[0].Type != model.EdgeTypeDrives {
...
func TestSQLiteStore_UpsertContentSubgraphSeedsVerifyQueueForPendingObjects(t *testing.T) {
	...
	subgraph.Edges = []model.ContentEdge{{
		ID:                 "e1",
		From:               "n1",
		To:                 "n2",
		Type:               model.EdgeTypeDrives,
		IsPrimary:          true,
		VerificationStatus: model.VerificationPending,
	}}
...
func TestSQLiteStore_ApplyVerifyVerdictToContentSubgraphUpdatesNodeAndEdgeStatuses(t *testing.T) {
	...
	subgraph := testContentSubgraph()
	subgraph.Nodes = append(subgraph.Nodes, model.ContentNode{
		ID:                 "n2",
		SourceArticleID:    "unit-graph-1",
		SourcePlatform:     "twitter",
		SourceExternalID:   "g1",
	...
	if err := store.ApplyVerifyVerdictToContentSubgraph(context.Background(), "twitter", "g1", model.VerifyVerdict{
	...
	if err := store.ApplyVerifyVerdictToContentSubgraph(context.Background(), "twitter", "g1", model.VerifyVerdict{
		...
		ObjectID:   "e1",
		Verdict:    model.VerificationDisproved,
	...
	if gotNode.VerificationStatus != model.VerificationProved {
	...
	if got.Edges[0].VerificationStatus != model.VerificationDisproved {
