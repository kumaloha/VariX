...
func TestSQLiteStore_RunGlobalMemoryOrganizationBuildsNeutralClustersAcrossSources(t *testing.T) {
		...
		Output: model.Output{
			...
			Graph: model.ReasoningGraph{
				Nodes: []model.GraphNode{
					{ID: "n1", Kind: model.NodeFact, Text: "油价会上升", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
					{ID: "n2", Kind: model.NodeExplicitCondition, Text: "若地缘冲突升级"},
					{ID: "n3", Kind: model.NodePrediction, Text: "未来几年风险资产承压", PredictionStartAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
				},
	...
	recordB := model.Record{
		...
		Output: model.Output{
			...
			Graph: model.ReasoningGraph{
				Nodes: []model.GraphNode{
					{ID: "n1", Kind: model.NodeFact, Text: "油价会下降", OccurredAt: time.Date(2026, 4, 11, 0, 0, 0, 0, time.UTC)},
					{ID: "n2", Kind: model.NodeImplicitCondition, Text: "供给收缩会改变油价走势", OccurredAt: time.Date(2026, 4, 11, 0, 0, 0, 0, time.UTC)},
				},
				Edges: []model.GraphEdge{
					{From: "n2", To: "n1", Kind: model.EdgeDerives},
				},
			},
			Details:    model.HiddenDetails{Caveats: []string{"detail"}},
			Confidence: "medium",
...
func TestSQLiteStore_RunGlobalMemoryOrganizationMergesCrossSourceSharedProposition(t *testing.T) {
	...
	recordA := model.Record{
		...
		Output: model.Output{
			...
			Graph: model.ReasoningGraph{
				Nodes: []model.GraphNode{
					{ID: "n1", Kind: model.NodeConclusion, Text: "石油美元回流正在削弱", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
					{ID: "n2", Kind: model.NodePrediction, Text: "未来几年美国资产承压", PredictionStartAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
				},
	...
	recordB := model.Record{
		...
		Output: model.Output{
			...
			Graph: model.ReasoningGraph{
				Nodes: []model.GraphNode{
					{ID: "n1", Kind: model.NodeConclusion, Text: "石油美元回流面临断裂风险", OccurredAt: time.Date(2026, 4, 11, 0, 0, 0, 0, time.UTC)},
					{ID: "n2", Kind: model.NodePrediction, Text: "未来几年金融资产承压", PredictionStartAt: time.Date(2026, 4, 11, 0, 0, 0, 0, time.UTC)},
				},
				Edges: []model.GraphEdge{{From: "n1", To: "n2", Kind: model.EdgeDerives}},
			},
			Details:    model.HiddenDetails{Caveats: []string{"detail"}},
...
func TestSQLiteStore_RunGlobalMemoryOrganizationDerivesHigherLevelMacroTheme(t *testing.T) {
	...
	recordA := model.Record{
		...
		Output: model.Output{
			...
			Graph: model.ReasoningGraph{
				Nodes: []model.GraphNode{
					{ID: "n1", Kind: model.NodeConclusion, Text: "石油美元循环面临断裂风险", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
					{ID: "n2", Kind: model.NodePrediction, Text: "未来几年美债美股承压", PredictionStartAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
				},
				Edges: []model.GraphEdge{{From: "n1", To: "n2", Kind: model.EdgeDerives}},
			},
			Details:    model.HiddenDetails{Caveats: []string{"detail"}},
	...
	recordB := model.Record{
		...
		Output: model.Output{
			...
			Graph: model.ReasoningGraph{
				Nodes: []model.GraphNode{
					{ID: "n1", Kind: model.NodeConclusion, Text: "私募信贷流动性风险正在上升", OccurredAt: time.Date(2026, 4, 11, 0, 0, 0, 0, time.UTC)},
					{ID: "n2", Kind: model.NodePrediction, Text: "未来几年华尔街可能遭遇挤兑冲击", PredictionStartAt: time.Date(2026, 4, 11, 0, 0, 0, 0, time.UTC)},
				},
				Edges: []model.GraphEdge{{From: "n1", To: "n2", Kind: model.EdgeDerives}},
			},
			Details:    model.HiddenDetails{Caveats: []string{"detail"}},
	...
	records := []model.Record{
		{
			...
			Output: model.Output{
				...
				Graph: model.ReasoningGraph{
					Nodes: []model.GraphNode{
						{ID: "n1", Kind: model.NodeConclusion, Text: "传统现金与债券资产的实际购买力将不可避免地下降，货币面临系统性贬值风险", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
						{ID: "n2", Kind: model.NodePrediction, Text: "未来几年未进行分散配置的投资者将面临财富缩水", PredictionStartAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
					},
					Edges: []model.GraphEdge{{From: "n1", To: "n2", Kind: model.EdgeDerives}},
				},
			},
		...
		},
		{
			...
			Output: model.Output{
				...
				Graph: model.ReasoningGraph{
					Nodes: []model.GraphNode{
						{ID: "n1", Kind: model.NodeConclusion, Text: "石油美元循环面临断裂风险，且私募信贷正积累流动性隐患", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
						{ID: "n2", Kind: model.NodePrediction, Text: "未来几年美债美股承压", PredictionStartAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
					},
					Edges: []model.GraphEdge{{From: "n1", To: "n2", Kind: model.EdgeDerives}},
				},
		...
		},
		{
			...
			Output: model.Output{
				...
				Graph: model.ReasoningGraph{
					Nodes: []model.GraphNode{
						{ID: "n1", Kind: model.NodeFact, Text: "美国就业市场逼近衰退临界点，AI冲击白领就业，高收入家庭消费收缩", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
						{ID: "n2", Kind: model.NodeConclusion, Text: "美国经济面临衰退风险", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
					},
					Edges: []model.GraphEdge{{From: "n1", To: "n2", Kind: model.EdgeDerives}},
				},
			},
		...
		},
		{
			...
			Output: model.Output{
				...
				Graph: model.ReasoningGraph{
					Nodes: []model.GraphNode{
						{ID: "n4", Kind: model.NodeExplicitCondition, Text: "若伊朗战争持续引发显著的大宗商品价格冲击与全球供应链重塑"},
						{ID: "n5", Kind: model.NodeExplicitCondition, Text: "若银行去监管化政策能够被妥善设计与执行"},
						{ID: "n6", Kind: model.NodeImplicitCondition, Text: "当前高资产价格环境在遭遇宏观负面冲击时将放大金融系统脆弱性", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
						{ID: "n7", Kind: model.NodeConclusion, Text: "宏观风险正在累积，金融体系安全取决于监管与资产价格韧性", OccurredAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
						{ID: "n9", Kind: model.NodePrediction, Text: "妥善实施的银行去监管将提升金融体系安全性并更好支持经济增长", PredictionStartAt: time.Date(2026, 4, 10, 0, 0, 0, 0, time.UTC)},
					},
					Edges: []model.GraphEdge{
						{From: "n6", To: "n7", Kind: model.EdgePositive},
						{From: "n5", To: "n9", Kind: model.EdgeDerives},
						{From: "n7", To: "n9", Kind: model.EdgeDerives},
					},
				},
			},
