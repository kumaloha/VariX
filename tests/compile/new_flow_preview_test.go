...
func TestPreviewNewFlowForThreeSamples(t *testing.T) {
	...
	for i, sample := range samples {
		...
		go func(idx int, sample struct {
			...
			raw, err := loadManualRawCapture(context.Background(), settings.ContentDBPath, sample.platform, sample.externalID)
			if err != nil {
			...
			result, err := client.PreviewFlow(itemCtx, BuildBundle(raw), opts)
			if err != nil {
