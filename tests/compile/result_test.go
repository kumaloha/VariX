...
func TestOutputMarshalJSONIncludesRenderedGraph(t *testing.T) {
	raw, err := json.Marshal(Output{
		Summary: "一句话总结",
		Graph: ReasoningGraph{
			Nodes: []GraphNode{
				{ID: "n1", Kind: NodeFact, Text: "事实A", OccurredAt: mustTime(t, "2026-04-14T00:00:00Z")},
	...
	if err := json.Unmarshal(raw, &got); err != nil {
		t.Fatalf("json.Unmarshal(Output) error = %v; raw=%s", err, raw)
	}
	if len(got.Graph.Nodes) != 2 || len(got.Graph.Edges) != 1 {
		t.Fatalf("Graph = %#v, want public JSON to preserve rendered graph", got.Graph)
	}
}
...
func TestParseOutputKeepsLegacyVerificationCompatibleWithLegacyFactVerificationFields(t *testing.T) {
	raw := `{
	  "summary":"一句话",
	  "graph":{
	    "nodes":[
	...
	assertLegacyFactVerificationStringField(t, out.Verification, "Version", "")
	assertLegacyFactVerificationStringField(t, out.Verification, "RolloutStage", "")
	assertLegacyFactVerificationSliceLen(t, out.Verification, "Passes", 0)
	if _, ok := tryResolveLegacyFactVerificationPath(out.Verification, []string{"CoverageSummary"}); ok {
		t.Fatal("legacy verification should not require legacy fact-verifier coverage summary")
	}
}

func TestParseOutputPreservesLegacyFactVerificationFactsMetadataAlongsideLegacyArrays(t *testing.T) {
	raw := `{
	  "summary":"一句话",
	  "graph":{
	    "nodes":[
	...
	assertLegacyFactVerificationStringField(t, out.Verification, "Version", "verify_v2")
	assertLegacyFactVerificationStringField(t, out.Verification, "RolloutStage", "facts_only")
	assertLegacyFactVerificationSliceLen(t, out.Verification, "Passes", 1)
	assertLegacyFactVerificationStringField(t, out.Verification, []string{"Passes", "0", "Kind"}, "fact")
	assertLegacyFactVerificationStringSlice(t, out.Verification, []string{"Passes", "0", "NodeIDs"}, []string{"n1", "n2"})
	assertLegacyFactVerificationBoolField(t, out.Verification, []string{"Passes", "0", "Coverage", "Valid"}, true)
	assertLegacyFactVerificationStringSlice(t, out.Verification, []string{"Passes", "0", "RetrievalSummary", "RetrievedNodeIDs"}, []string{"n1"})
	assertLegacyFactVerificationStringSlice(t, out.Verification, []string{"Passes", "0", "RetrievalSummary", "NoResultNodeIDs"}, []string{"n2"})
	assertLegacyFactVerificationBoolField(t, out.Verification, []string{"Passes", "0", "RetrievalSummary", "ExcerptTruncated"}, true)
	assertLegacyFactVerificationStringField(t, out.Verification, []string{"Passes", "0", "Claim", "Model"}, "claim-model")
	assertLegacyFactVerificationStringField(t, out.Verification, []string{"Passes", "0", "Challenge", "Model"}, "challenge-model")
	assertLegacyFactVerificationStringField(t, out.Verification, []string{"Passes", "0", "Adjudication", "Model"}, "judge-model")
	assertLegacyFactVerificationTimeField(t, out.Verification, []string{"Passes", "0", "Adjudication", "CompletedAt"}, mustTime(t, "2026-04-15T03:00:00Z"))
	assertLegacyFactVerificationBoolField(t, out.Verification, []string{"CoverageSummary", "Valid"}, true)
	assertLegacyFactVerificationIntField(t, out.Verification, []string{"CoverageSummary", "TotalExpectedNodes"}, 2)
	assertLegacyFactVerificationIntField(t, out.Verification, []string{"CoverageSummary", "TotalFinalizedNodes"}, 2)
}

func TestOutputValidateAcceptsLegacyFactVerificationMixedAdjudicationMetadataWithEmptyTopLevelModel(t *testing.T) {
	raw := `{
	  "summary":"一句话",
	  "graph":{
	    "nodes":[
	...
	assertLegacyFactVerificationStringField(t, out.Verification, []string{"Passes", "0", "Adjudication", "Model"}, "judge-a")
	assertLegacyFactVerificationStringField(t, out.Verification, []string{"Passes", "1", "Adjudication", "Model"}, "judge-b")
	assertLegacyFactVerificationTimeField(t, out.Verification, []string{"Passes", "1", "Adjudication", "CompletedAt"}, mustTime(t, "2026-04-15T04:00:00Z"))
}

func assertLegacyFactVerificationFieldPresent(t *testing.T, root any, path any) {
	t.Helper()
	if _, ok := tryResolveLegacyFactVerificationPath(root, normalizeLegacyFactVerificationPath(path)); !ok {
		t.Fatalf("missing legacy fact-verifier field at path %v", normalizeLegacyFactVerificationPath(path))
	}
}

func assertLegacyFactVerificationOneOfStringSlices(t *testing.T, root any, candidatePaths [][]string, want []string) {
	t.Helper()
	for _, path := range candidatePaths {
		if got, ok := tryResolveLegacyFactVerificationPath(root, path); ok && got.IsValid() && got.Kind() == reflect.Slice {
			if got.Len() != len(want) {
				continue
			}
			matched := true
	...
	t.Fatalf("none of the candidate legacy fact-verifier paths matched %v: %v", want, candidatePaths)
}

func assertLegacyFactVerificationStringField(t *testing.T, root any, path any, want string) {
	t.Helper()
	got := mustResolveLegacyFactVerificationPath(t, root, path)
	if got.Kind() != reflect.String {
		t.Fatalf("path %v kind = %s, want string", normalizeLegacyFactVerificationPath(path), got.Kind())
	}
	if got.String() != want {
		t.Fatalf("path %v = %q, want %q", normalizeLegacyFactVerificationPath(path), got.String(), want)
	}
}

func assertLegacyFactVerificationBoolField(t *testing.T, root any, path any, want bool) {
	t.Helper()
	got := mustResolveLegacyFactVerificationPath(t, root, path)
	if got.Kind() != reflect.Bool {
		t.Fatalf("path %v kind = %s, want bool", normalizeLegacyFactVerificationPath(path), got.Kind())
	}
	if got.Bool() != want {
		t.Fatalf("path %v = %v, want %v", normalizeLegacyFactVerificationPath(path), got.Bool(), want)
	}
}

func assertLegacyFactVerificationTimeField(t *testing.T, root any, path any, want time.Time) {
	t.Helper()
	got := mustResolveLegacyFactVerificationPath(t, root, path)
	if got.Type() != reflect.TypeOf(time.Time{}) {
		t.Fatalf("path %v type = %s, want time.Time", normalizeLegacyFactVerificationPath(path), got.Type())
	}
	if !got.Interface().(time.Time).Equal(want) {
		t.Fatalf("path %v = %v, want %v", normalizeLegacyFactVerificationPath(path), got.Interface(), want)
	}
}

func assertLegacyFactVerificationIntField(t *testing.T, root any, path any, want int) {
	t.Helper()
	got := mustResolveLegacyFactVerificationPath(t, root, path)
	if got.Kind() != reflect.Int {
		t.Fatalf("path %v kind = %s, want int", normalizeLegacyFactVerificationPath(path), got.Kind())
	}
	if int(got.Int()) != want {
		t.Fatalf("path %v = %d, want %d", normalizeLegacyFactVerificationPath(path), got.Int(), want)
	}
}

func assertLegacyFactVerificationSliceLen(t *testing.T, root any, field string, want int) {
	t.Helper()
	got := mustResolveLegacyFactVerificationPath(t, root, []string{field})
...
func assertLegacyFactVerificationStringSlice(t *testing.T, root any, path []string, want []string) {
	t.Helper()
	got := mustResolveLegacyFactVerificationPath(t, root, path)
	if got.Kind() != reflect.Slice {
...
func tryResolveLegacyFactVerificationPath(root any, parts []string) (reflect.Value, bool) {
...
func mustResolveLegacyFactVerificationPath(t *testing.T, root any, path any) reflect.Value {
	t.Helper()
	parts := normalizeLegacyFactVerificationPath(path)
	value := reflect.ValueOf(root)
	for _, part := range parts {
		...
		field := value.FieldByName(part)
		if !field.IsValid() {
			t.Fatalf("missing legacy fact-verifier field %q at path %v", part, parts)
		}
		value = field
	}
	for value.Kind() == reflect.Pointer {
...
func normalizeLegacyFactVerificationPath(path any) []string {
	switch v := path.(type) {
	case string:
		return []string{v}
	case []string:
	...
	default:
		panic("unsupported legacy fact-verifier path type")
	}
}
