...
func TestAuthorValidatePreviewResultValidatesAuthorOnlyWithSearch(t *testing.T) {
	...
	result, err := client.AuthorValidatePreviewResult(context.Background(), Bundle{
		...
		Render: Output{
			...
			TransmissionPaths: []TransmissionPath{{Driver: "Rates fall", Steps: []string{"Liquidity improves"}, Target: "Stocks will rise"}},
		},
	})
	if err != nil {
	...
	if got := result.Render.AuthorValidation.ClaimChecks[0].RequiredEvidence; len(got) != 1 || got[0].Metric != "rate change" || got[0].Status != AuthorClaimSupported {
		t.Fatalf("claim required evidence = %#v, want supported rate-change requirement", got)
	}
	if got := result.Render.AuthorValidation.InferenceChecks[0].RequiredEvidence; len(got) != 1 || got[0].SourceType != "market_data" || got[0].Status != AuthorClaimUnverified {
...
func TestAuthorValidatePreviewResultIncludesExternalEvidenceHints(t *testing.T) {
	...
	if got := result.AuthorValidation.ClaimChecks[0].Status; got != AuthorClaimSupported {
...
func TestNormalizeAuthorVerificationPlanEnrichesExecutableMarketDataSpecs(t *testing.T) {
		...
		ClaimPlans: []authorClaimVerificationPlan{
			{
				...
				RequiredEvidence: []AuthorEvidenceRequirement{{
	...
	byID := map[string]AuthorEvidenceRequirement{}
...
func TestCollectAuthorClaimCandidatesUsesOnlyRenderNodesAndEvidence(t *testing.T) {
	claims := collectAuthorClaimCandidates(Output{
		Drivers:            []string{"Rates fall"},
...
func TestCollectAuthorInferenceCandidatesCarriesEdgeProvenance(t *testing.T) {
	candidates := collectAuthorInferenceCandidates(Output{
		TransmissionPaths: []TransmissionPath{{
			Driver: "Rates fall",
			Steps:  []string{"Liquidity improves"},
			Target: "Stocks rise",
...
func TestAuthorValidationBackfillsPreviewGraphProvenanceForOldRender(t *testing.T) {
	result := FlowPreviewResult{
		...
		Render: Output{
			...
			Details: HiddenDetails{Caveats: []string{"old render without details.items"}},
		},
	}
...
func TestNormalizeAuthorValidationBackfillsMissingCandidates(t *testing.T) {
	validation := normalizeAuthorValidation(AuthorValidation{
		ClaimChecks: []AuthorClaimCheck{{ClaimID: "claim-001", Status: AuthorClaimSupported}},
	...
	if validation.ClaimChecks[1].Status != AuthorClaimUnverified {
		t.Fatalf("missing claim status = %q, want unverified", validation.ClaimChecks[1].Status)
	}
	if len(validation.InferenceChecks) != 1 || validation.InferenceChecks[0].Status != AuthorInferenceWeak {
...
func TestNormalizeAuthorValidationRejectsAuthorOnlySupportedClaimEvidence(t *testing.T) {
	validation := normalizeAuthorValidation(AuthorValidation{
		ClaimChecks: []AuthorClaimCheck{{
			ClaimID:   "claim-001",
			Text:      "中东石油减产量达每天910万桶",
			ClaimType: "number",
			Status:    AuthorClaimSupported,
			RequiredEvidence: []AuthorEvidenceRequirement{{
...
func TestNormalizeAuthorValidationPreservesSupportedClaimWithExternalEvidence(t *testing.T) {
	...
	if validation.ClaimChecks[0].Status != AuthorClaimSupported {
...
func TestNormalizeAuthorValidationRejectsVagueExternalSupportedEvidence(t *testing.T) {
	...
	if validation.ClaimChecks[0].Status != AuthorClaimUnverified {
...
func TestNormalizeAuthorValidationUsesFREDHintForNumericSupport(t *testing.T) {
	validation := normalizeAuthorValidationWithHints(AuthorValidation{
		ClaimChecks: []AuthorClaimCheck{{
			ClaimID:   "claim-001",
	...
	check := validation.ClaimChecks[0]
	if check.Status != AuthorClaimSupported {
...
func TestNormalizeAuthorValidationUsesStablecoinHintForContradiction(t *testing.T) {
	...
	check := validation.ClaimChecks[0]
	if check.Status != AuthorClaimContradicted {
...
func TestNormalizeAuthorValidationDowngradesAuthorOnlySoundInferenceAndDedupes(t *testing.T) {
	validation := normalizeAuthorValidation(AuthorValidation{
		InferenceChecks: []AuthorInferenceCheck{
			{
				InferenceID: "inference-001",
				From:        "全球石油供应缺口约7-10%",
				Steps:       []string{"美国财政应对空间有限", "美联储大概率选择印钞"},
				To:          "对冲通胀资产上涨",
				Status:      AuthorInferenceSound,
				RequiredEvidence: []AuthorEvidenceRequirement{{
	...
	if check.Status != AuthorInferenceWeak {
...
func TestNormalizeAuthorValidationBackfillsNarrativeCandidatesAsInterpretive(t *testing.T) {
	validation := normalizeAuthorValidation(AuthorValidation{}, []authorClaimCandidate{
	...
	if validation.ClaimChecks[0].Status != AuthorClaimInterpretive {
...
func TestNormalizeAuthorValidationPreservesModelSplitClaims(t *testing.T) {
		...
		ClaimChecks: []AuthorClaimCheck{
			{
				...
				Subclaims: []AuthorSubclaim{
					...
					},
					{
						...
						AttributionOK: false,
						Status:        AuthorClaimContradicted,
	...
	if validation.ClaimChecks[0].Status != AuthorClaimContradicted {
...
func TestNormalizeAuthorValidationKeepsInterpretiveParentWithSupportedSubclaims(t *testing.T) {
	validation := normalizeAuthorValidation(AuthorValidation{
		ClaimChecks: []AuthorClaimCheck{{
			...
			Status:    AuthorClaimInterpretive,
			Subclaims: []AuthorSubclaim{{
