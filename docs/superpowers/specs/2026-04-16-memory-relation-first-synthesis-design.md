# VariX Memory synthesis 关系优先认知系统设计（终版草案）

## 1. Goal

VariX Memory synthesis 不是文章归档系统，也不是 topic 聚类系统，而是一个**以财经传导关系为核心的认知数据库**。
...
Memory synthesis 的底层关系 truth boundary 不是 thesis，也不是 cluster，而是 **Relation**。
...
### Phase 1: coexistence
- 保留 cluster-first `cluster-first`
- 新增 synthesis `relation-first`
- accepted memory truth 不变
- compare surface 同时支持 cluster / synthesis
...
- relation-first synthesis 成为默认 mental model
- cluster-first 仅作为兼容与调试工具存在
...
> **VariX Memory synthesis 是一个以 CanonicalEntity 为锚点、以 Relation 为关系边界、以 Mechanism 为传导正文、以 driver / target 双聚合为导航层、以卡片 / 结论 / 冲突为产品层的财经认知系统。**
